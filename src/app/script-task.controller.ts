...
export class ScriptTaskController {
  ...
  async get(@Body() body: any): Promise<any> {
    try {
      const data = await this.database.scriptTask.findOne(body.id);
      return data ? {
        error: 0,
        data,
      } : {
  ...
  async lists(@Body() body: any): Promise<any> {
    try {
      const size = await this.database.scriptTask.count();
      const data = await this.database.scriptTask.find({
  ...
  async add(@Body() body: any): Promise<any> {
    try {
      const result = await this.database.scriptTask.insert({
  ...
  async update(@Body() body: any): Promise<any> {
    try {
      ...
      await this.database.scriptTask.update(id, body);
      return {
        error: 0,
        msg: 'ok',
      };
  ...
  async delete(@Body() body: any): Promise<any> {
    try {
      await this.database.scriptTask.delete(body.id);
      return {
        error: 0,
        msg: 'ok',
      };
